...
define('CLIENT_VERSION', '1.5.0');	//note UCenter 版本标识
define('CLIENT_RELEASE', '20081031');
...
if(!defined('IN_UC')) {
	...
	$code = @$_GET['code'];
	parse_str(_authcode($code, 'DECODE', KEY), $get);
	if(MAGIC_QUOTES_GPC) {
	...
	require_once DISCUZ_ROOT.'./client/lib/xml.class.php';
...
class note {
	...
	function _serialize($arr, $htmlon = 0) {
		if(!function_exists('xml_serialize')) {
			include_once DISCUZ_ROOT.'./client/lib/xml.class.php';
		}
	...
	function updatehosts($get, $post) {
		...
		$cachefile = $this->appdir.'./client/data/cache/hosts.php';
		$fp = fopen($cachefile, 'w');
		$s = "<?php\r\n";
	...
	function updateapps($get, $post) {
		...
		//note 写 app 缓存文件
		$cachefile = $this->appdir.'./client/data/cache/apps.php';
		$fp = fopen($cachefile, 'w');
		$s = "<?php\r\n";
		...
		if(is_writeable($this->appdir.'./config.inc.php')) {
			...
			$configfile = preg_replace("/define\('API',\s*'.*?'\);/i", "define('API', '$API');", $configfile);
	...
	function updateclient($get, $post) {
		...
		$cachefile = $this->appdir.'./client/data/cache/settings.php';
		$fp = fopen($cachefile, 'w');
		$s = "<?php\r\n";
