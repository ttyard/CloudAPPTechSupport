 ...
 * 使用到的接口函数：
 * get_user()	必须，获取用户的信息
 * user_register()	必须，注册用户数据
 * authcode()	可选，借用用户中心的函数加解密激活字串和 Cookie
 */
...
if(empty($_POST['submit'])) {
	...
	if($_GET['action'] == 'activation') {
		echo '激活:';
		list($activeuser) = explode("\t", authcode($_GET['auth'], 'DECODE'));
	...
	//在UCenter注册用户信息
	$username = '';
	if(!empty($_POST['activation']) && ($activeuser = get_user($_POST['activation']))) {
		list($uid, $username) = $activeuser;
	} else {
		if(get_user($_POST['username']) && !$db->result_first("SELECT uid FROM {$tablepre}members WHERE username='$_POST[username]'")) {
			//判断需要注册的用户如果是需要激活的用户，则需跳转到登录页面验证
			echo '该用户无需注册，请激活该用户<br><a href="'.$_SERVER['PHP_SELF'].'?example=login">继续</a>';
			exit;
		}

		$uid = user_register($_POST['username'], $_POST['password'], $_POST['email']);
		if($uid <= 0) {
			if($uid == -1) {
				echo '用户名不合法';
			} elseif($uid == -2) {
	...
	if($username) {
		...
		setcookie('Example_auth', authcode($uid."\t".$username, 'ENCODE'));
