...
switch($action) {
	case '':
		...
		$data = pm_list($Example_uid, $_GET['page'], $ppp, $_GET['folder'], $_GET['filter'], 100);
	...
	case 'view':
		...
		$data = pm_view($Example_uid, $pmid, $_GET['touid']);
		...
		break;
	case 'delete':
		if(pm_deleteuser($Example_uid, array($_GET['uid']))) {
			$output .= "短消息已删除。";
		}
		break;
	case 'addblackls':
		$user = !empty($_GET['user']) ? $_GET['user'] : (!empty($_POST['user']) ? $_POST['user'] : '');
		if(pm_blackls_add($Example_uid, $user)) {
			$output .= $_GET['user']." 已加入黑名单。";
		}
		break;
	case 'deleteblackls':
		if(pm_blackls_delete($Example_uid, $_GET['user'])) {
			$output .= $_GET['user']." 已从黑名单中移除。";
		}
		break;
	case 'viewblackls':
		$data = explode(',', pm_blackls_get($Example_uid));
		foreach($data as $ls) {
	...
	case 'send':
		if(!empty($_POST)) {
			...
			if(pm_send($Example_uid, $msgto, $_POST['subject'], $_POST['message'], 1, 0, $isusername)) {
				$output .= "短消息已发送";
			} else {
