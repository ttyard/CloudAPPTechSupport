 ...
 * 列出好友的 Example 代码
 * 使用到的接口函数：
 * friend_totalnum()	必须，返回好友总数
 * friend_ls()	必须，返回好友列表
 * friend_delete()	必须，删除好友
 * friend_add()	必须，添加好友
 */

if(empty($_POST['submit'])) {
	$num = friend_totalnum($Example_uid);
	echo '您有 '.$num.' 个好友';
	echo '<form method="post" action="'.$_SERVER['PHP_SELF'].'?example=friend">';
	$friendlist = friend_ls($Example_uid, 1, 999, $num);
	if($friendlist) {
	...
	if(!empty($_POST['delete']) && is_array($_POST['delete'])) {
		friend_delete($Example_uid, $_POST['delete']);
	}
	if($_POST['newfriend'] && $friendid = get_user($_POST['newfriend'])) {
		friend_add($Example_uid, $friendid[0], $_POST['newcomment']);
	}
